...
class Namespace(object):
    ...
    def _add_member(self, member):
        if isinstance(member, Version):
            ...
            if member.name in (self.arrays or self.constants or self.enumerations or self.maps or self.structs or self.typedefs):
                raise ASTException(
...
class Interface(Namespace):
    ...
    def _add_member(self, member):
        if isinstance(member, Type):
            #print((member.name in (self.attributes or self.methods or self.broadcasts)))
            if ((member.name in self.attributes and isinstance(member, Attribute)) or (member.name in self.methods and isinstance(member, Method)) or \
                (member.name in self.broadcasts and isinstance(member, Broadcast))) :
                raise ASTException(
                    "Interface: Duplicate namespace member '{}'.".format(member.name))
            #else:
